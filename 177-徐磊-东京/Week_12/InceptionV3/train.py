...
def data_load(batch_size):
    data_transform = {
        'train': transfroms.Compose([
            transfroms.RandomResizedCrop(299),
...
class Engine(object):
    ...
    def train(self, train_loader):
        ...
        for images, labels in train_loader:
            ...
            outputs, outputs_aux = self.net(images)
            loss1 = self.criterion(outputs, labels)
            if outputs_aux is not None:
                loss2 = self.criterion(outputs_aux, labels)
            else:
                loss2 = 0
